...
class CorrelationShiftPicture:
    _subpath = "corr_shift_3s_each1s"
    ...
    def __init__(self):
        ...
        batches = self._collect_batches(
            ...
            days=MDP.days_unpack([3]),
            names=MDP.names_unpack(["h", "emt"]),
        )

        self._plot_batches(batches, 3, 5)
    ...
    def _plot_batches(self, batches, image_shifts, cols, freq=100):
        for idx, batch in enumerate(batches):
            ...
            print(
                f"saved: {idx+1}/{len(batches)} in: {datetime.now()-time} total: {datetime.now()-self._start_time} {batch[0].get_id()}"
            )
    ...
    def _plot_batch(self, batch, image_shifts, cols, freq):
        ...
        for image_shift in range(image_shifts):
            ...
            for rec in recs:
                ...
                for i in range(cols):
                    start_time = (i * 3000) + 3000 + (image_shift * cols * 3000)
                    offset = 0
                    length = 100
